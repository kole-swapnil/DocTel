...
const distributeUsers = () => {
    let remainingUsers = 340;
	//[35, 50, 70, 100, 140, 200, 280, 340]
    let userDistribution = new Array(RPC_URLS.length).fill(12); // minimum 40 users per URL
    remainingUsers -= RPC_URLS.length * 7; // Subtract the allocated
...
const runSingleTest = async () => {
    ...
    console.log(`Trans distribution:`, RPC_URLS.map((url, i) => `${url}: ${userDistribution[i]} trans`).join(" | "));
    ...
    console.log(`Total time for trans: ${totalTime} ms`);
    sleep(5000);
    return totalTime;
};
